...
class FLENLayer(nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim, sparse_inputs_slots,
                 sparse_num_field, layer_sizes_dnn):
        ...
        self._EmbeddingLayer = EmbeddingLayer(sparse_feature_number, sparse_feature_dim, sparse_num_field)
        self._DNNLayer = DNNLayer(sparse_feature_dim, sparse_inputs_slots, layer_sizes_dnn)
        self._FieldWiseBiInteraction = FieldWiseBiInteraction(sparse_feature_dim, sparse_num_field)
...
class EmbeddingLayer(nn.Layer):
    def __init__(self, sparse_feature_number, sparse_feature_dim, sparse_num_field):
...
class DNNLayer(nn.Layer):
    def __init__(self, sparse_feature_dim, sparse_inputs_slots, layer_sizes_dnn, dropout_rate=0.2):
...
class FieldWiseBiInteraction(nn.Layer):
    def __init__(self, sparse_feature_dim, num_fields, activation=None, use_bias=False):
    ...
    def forward(self, inputs):
        ...
        embeddings_right = paddle.gather(field_wise_vectors, index=right, axis=1)

        embeddings_prod = paddle.multiply(embeddings_left, embeddings_right)
        field_weighted_embedding = paddle.multiply(embeddings_prod, self.kernel_mf)
