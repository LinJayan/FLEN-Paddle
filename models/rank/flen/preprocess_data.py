...
class PreDataset(object):
    ...
    def _build_split(self):
        ...
        train_file = open(os.path.join(self.config.get("runner.train_data_dir"),'train_data.txt'),'w')
        test_file = open(os.path.join(self.config.get("runner.test_data_dir"),'test_data.txt'),'w')
        ...
        for file in [self.file_path]:
            with open(file, "r") as rf:
                ...
                pbar.set_description('Split avazu dataset: train_dataset and test_dataset')
                for line in pbar:
                    sample_cnt += 1
                    ...
                    features = {self.idx_to_field_name[idx]: feature_mapper[self.idx_to_field_name[idx]][value] for idx, value in enumerate(values)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Parameter of preprocess data")
